...
import { SessionSettingsManager } from "../../components/SessionSettingsManager";
...
export default async function SessionSettingsPage() {
  ...
  const sidebarItems = isSuperAdmin ? adminNavItems("session") : userNavItems("home");
  ...
  return (
    ...
    >
      ...
      {isSuperAdmin && <Breadcrumb items={adminCrumb("Session Timeout")} />}
      <h1 className="iipe-page-title">Session Timeout</h1>

      {!isSuperAdmin ? (
        <div className="iipe-card">
          <h2>Super Admin only</h2>
          <p style={{ marginTop: 0 }}>
            Configuring session timeout is restricted to the Super Admin.
          </p>
